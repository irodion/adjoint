...
1. **Hook scripts** — installed as pinned console binaries (`adjoint-hook-*`) that Codex invokes per lifecycle event. Budgets are tight (300 ms – 2 s); every hook **fails open**. Never treat a hook failure as blocking; adjoint is **not a security boundary**.
...
PreCompact / SessionEnd → spawn detached `adjoint memory flush` via Popen(start_new_session=True)
     flush → last-30-turns / 15 000-char tail of transcript → redact (pre-LLM) → Codex Agent SDK call (allowed_tools=[]) → redact (post-LLM) → append to daily/YYYY-MM-DD.md
...
- `memory/agent.AgentRequest.recursion_tag` → propagated into `Codex-agent-sdk` via `env=`.
...
`src/adjoint/bundled/settings.hooks.json` and `settings.mcp.json` are merged into the target `.Codex/settings.json` by `install.build_install_plan` + `apply_install`:
...
`cli.py` does rich output + typer glue; all work happens in `memory.*`, `install`, `providers.*`. Module-level imports in `cli.py` are kept minimal so `--version` and `--help` don't pull in `Codex-agent-sdk`. The `from .memory.{flush,compile,query,lint} import …` statements live **inside** the command handlers by design.
...
- `tests/fake_agent.py::FakeAgent` implements the `AgentClient` protocol with a response queue — any memory test that would otherwise call `Codex-agent-sdk` must pass `client=FakeAgent().enqueue(...)` into `flush()` / `compile_project()` / `lint()` / etc.
